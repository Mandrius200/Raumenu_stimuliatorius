...
//----------------------------------------------------------------------------------------MAIN MENU--------------------------------------------------------------
    case MAIN_MENU:
      ...
      readButtons();
      if (last_pos != sel_pos || last_SOC!=SOC){
        display.clearDisplay();
        displayMessage(1, 1, batteryCharge);
        displayMessage(10, 30, menu_1[0]);
        displayMessage(10, 50, menu_1[1]);
...
//-------------------------------------------------------------------------------------------STIM MENU-------------------------------------------------------------------
  case STIM_MENU:
    ...
    if (drawStatus == 0){
      ...
      readButtons();
      if (last_start_stop != start_stop || last_intensity != intensity || last_SOC != SOC){
        display.clearDisplay();
        displayMessage(1, 1, batteryCharge);
        displayMessage(5, 20, "STATUS:");
...
void handleWebSocketMessage(void *arg, uint8_t *data, size_t len) {
  ...
  if (info->final && info->index == 0 && info->len == len && info->opcode == WS_TEXT) {
    ...
    if (message.indexOf("1s") >= 0) {
      ...
      frequency_slider = map(sliderValue1.toInt(), 1, 100, 2, 100);
    ...
    if (message.indexOf("2s") >= 0) {
      ...
      pulse_width_slider = map(sliderValue2.toInt(), 400, 1000, 400, 1000);
    ...
    if (message.indexOf("3s") >= 0) {
      ...
      intensity_slider = map(sliderValue3.toInt(), 1, 100, 1, 100);
...
void stim_mode_1(uint intensity){
  unsigned long periodMicros = 50000;
  unsigned long microsTime = micros();
  unsigned long pulseMicros = 420;
  static unsigned long startTime = 0;
  int pos_pulse = zero_level + intensity;
  int neg_pulse = zero_level - intensity;

  // if (pulse_status==false){
  //   pulse_status = true;
  //   setV(pos_pulse);
  //   delayMicroseconds(pulseMicros-340);
  //   setV(neg_pulse);
  //   delayMicroseconds(pulseMicros-340);
  // } 
  if (microsTime - startTime < 2*pulseMicros /*positive_pulse == false*/) {

    setV(pos_pulse);
    delayMicroseconds(pulseMicros-340);
    setV(neg_pulse);
    delayMicroseconds(pulseMicros-419);
  }
  //else if (pulseMicros < microsTime - startTime && microsTime - startTime < 2 * pulseMicros) setV(neg_pulse);
  else  setV(zero_level);
...
void stim_mode_wifi(uint frequency, uint pulse_width, uint intensity){
  ...
  //255 max +
  //0 max -
  int pos_pulse = 112 + intensity;
  int neg_pulse = 112 - intensity;
  ...
  //} else if (pulseMicros < microsTime - startTime && microsTime - startTime < 2 * pulseMicros /*negative_pulse == false */) {
    ...
    //setV(neg_pulse);
    //Serial.println(microsTime-startTime);
